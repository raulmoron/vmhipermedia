...
class sqlAPI extends API
{
    ...
    /**
     * Execute any query.
     * curl -X POST --data "sql={sql}" {domain}/query
     */
    public function query(){
        if( "PUT" === $this->method){
            ...
            try{ 
                ...
                $return = array(
                    "status"        => "ok",
                    "response"      => (0 == $this->db->getAffectedRows())? $response->getData() : "",
                    "debug"         => array(
                        "query"         => $sql,
     ...
     *
     * Insert music: POST {domain}/music
     * curl -X POST --data "artist={artist}&title={title}&track_id={track_id}&playlist_id={playlist_id}" {domain}/music
     */
    public function music($args){
        require_once 'libs/models/music.php';
...
try {
    ...
    {
        throw new Exception_302("/doc/index.html");
    }
